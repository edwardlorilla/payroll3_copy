    ...
    <aside class="page-sidebar">
        <div class="page-logo">
            <a href="#" class="page-logo-link press-scale-down d-flex align-items-center position-relative" data-toggle="modal" data-target="#modal-shortcut">
                <img  style="height:3.125rem; width: 3.125rem;" :src="$store.getters.SETTING && $store.getters.SETTING.image ?  `/upload/${$store.getters.SETTING.image.url}`:'/img/favicon/favicon-32x32.png'" alt="Payroll" aria-roledescription="logo">
                <span class="page-logo-text mr-1">{{$store.getters.SETTING ? $store.getters.SETTING.name : 'PAYROLL'}}</span>
        ...
        <nav id="js-primary-nav" class="primary-nav" role="navigation">
            <div class="nav-filter">
                <div class="position-relative">
                    <input type="text" id="nav_filter_input" placeholder="Filter menu" v-model="filterKey" class="form-control" tabindex="0">
            ...
            <div class="info-card">
                <img @click="$router.push({name: 'profile.edit', params:{id: $store.getters.USER.id}})" style="height:3.125rem; width: 3.125rem;" :src="$store.getters.USER.image ?`/upload/100_${$store.getters.USER.image.url}` : '/img/demo/avatars/avatar-m.png'" class="profile-image rounded-circle" alt="avatar">
                <div class="info-card-text">
                    <router-link :to="{name: 'profile.edit', params:{id: $store.getters.USER.id}}" class="d-flex align-items-center text-white">
                        <span class="text-truncate text-truncate-sm d-inline-block">
                                       {{$store.getters.USER.name ? $store.getters.USER.name : ''}}
                                    </span>
                    </router-link>
                    <span class="d-inline-block text-truncate text-truncate-sm">{{$store.getters.USER.address ? $store.getters.USER.address : ''}}</span>
                </div>
            ...
            <ul id="js-nav-menu" class="nav-menu">
                <li v-for="link in filteredNavigation" v-if="link.can ? can[link.can] : true">
                    <router-link class="nav-link-text" :to="{name: link.name}">{{link.label}}</router-link>
                </li>
                <!--<li>
    ...
    export default {
        ...
        computed:{
            filteredNavigation() {
                ...
                if (filterKey) {
                    navigations = navigations.filter(function (row) {
