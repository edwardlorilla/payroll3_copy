...
var initApp = (function(app) {
	...
	app.listFilter = function (list, input, anchor) {
		...
		$(input).change( function () {
			...
			/* when users types more than 1 letter start search filter */
			if(filter.length > 1) {
	 ...
	 **/
	 app.checkNavigationOrientation = function() {
		...
		switch ( true ) {
			...
			/* fires when users switches to nav-function-top on desktop view */
			...
			/* fires when users resizes screen to mobile size or app is loaded on mobile resolution */
	...
	app.domReadyMisc = function() {
		...
		if ( typeof $.fn.slimScroll !== 'undefined' && myapp_config.thisDevice === 'desktop') {
			...
			console.log("WARN! $.fn.slimScroll not loaded or users is on desktop");
