	...
	root.smartSpeechRecognition = {
		...
		init : function(commands, resetCommands) {
			...
			recognition.onerror = function(event) {
				...
				switch (event.error) {
					...
					case 'service-not-allowed':
						...
						//debugState
						if (debugState) {
							root.console.log('%c WARNING: Microphone was not detected (either users denied access or it is not installed properly) ', debugStyle_warning);
						}
						// determine if permission was denied by users or automatically.
			...
			recognition.onend = function() {
				...
				// smartSpeechRecognition will auto restart if it is closed automatically and not by users action.
				if (autoRestart) {
		...
		// Set the language the users will speak in. If not called, defaults to 'en-US'.
		...
		// Lets the users add a callback of one of 9 types:
